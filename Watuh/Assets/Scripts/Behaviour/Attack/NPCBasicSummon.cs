...
public class NPCBasicSummon : MonoBehaviour, INPCAttackState
{
    [SerializeField]
    ...
    [SerializeField]
    private GameObject _thingToSummon;
    [SerializeField]
    private float _summonInFrontDistance = 1;
    [SerializeField]
    ...
    private IEnumerator ShootFlyRoutine()
    {
        ...
        yield return new WaitForSeconds((_timeShootAnimation/3)*2);
        GameObject projectile = Instantiate(_thingToSummon, transform.position + transform.forward * _summonInFrontDistance, Quaternion.identity);
        yield return new WaitForSeconds((_timeShootAnimation / 3) * 1);
